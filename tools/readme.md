...
### Metasploit Framework — exploitation & post-exploit framework
**What it is:** A modular platform for developing, testing, and running exploits and payloads, plus post-exploitation modules and helpers.  
**Role:** Offensive automation and exploitation — convert verified vulnerabilities into controlled proofs-of-concept and manage post-exploit tasks.

**How to use (conceptual)**  
1. **Confirm vulnerability** via recon or manual testing.  
2. **Search** for matching exploit modules.  
3. **Configure** module options (RHOSTS, LHOST, payload, etc.).  
4. **Test in an isolated lab** and run the exploit.  
5. **Manage sessions** for post-exploit actions (pivoting, data collection).  
...
- **Metasploit:** Use for exploitation proofs and session management after you’ve validated a vulnerability—always in an isolated lab first.
...
| **Web application recon**      | `Burp Suite`, `ffuf`                                               | Intercept/inspect traffic and find hidden endpoints.          |
| **Credential assessment**      | `Hydra`                                                            | Test password strength and authentication policies.           |
| **Exploit validation**         | `Metasploit Framework`                                             | Turn vulnerabilities into controlled exploits/proofs.         |
